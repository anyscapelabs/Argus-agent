...
import { RiAiAgentLine, RiBrainLine, RiToolsLine } from "react-icons/ri";
...
export default function Sidebar({
  ...
  return (
    <aside
      className={`flex h-full shrink-0 flex-col overflow-hidden bg-bg-primary transition-[width] duration-300 ease-in-out ${
    ...
    >
      ...
      <button
        type="button"
        onClick={onNewAgent}
        className={`mx-2 mt-1 flex items-center gap-2 rounded-md px-2 py-1 transition-colors focus:outline-none ${
          activeView === "new-agent"
            ? "bg-bg-hover-primary text-text-primary"
            : "text-text-secondary hover:bg-bg-hover-primary"
        }`}
      >
        <RiAiAgentLine size={16} className={activeView === "new-agent" ? "text-text-primary" : "text-text-secondary"} />
        ...
        {TABS.map(({ label, view, Icon }) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 rounded-md px-2 py-1 transition-colors focus:outline-none ${
                isActive
                  ? "bg-bg-hover-primary text-text-primary"
                  : "text-text-secondary hover:bg-bg-hover-primary focus-visible:bg-bg-hover-primary"
              }`}
            >
              <Icon size={16} className={isActive ? "text-text-primary" : "text-text-secondary"} />
