...
- `terminal|bash.run` (mutating, 120s cap, 10–1800s timeout, cwd, `Channel` streaming), `grep` (read-only recursive + globs), `fs.write` (mutating), `doc.create`, `skill.*`, `memory.*`, plus `WEB_TOOLS: web.search|web.read`.
...
- `shell/mod.rs:61 run_stream(chan)` — live `term|term_end` chunks.
- `fs/mod.rs:5 write()`, `grep/mod.rs:8 run()`.
...
- Read-only tools never require approval; mutating tools always go through the gate. Keep `terminal` 120s default cap.
