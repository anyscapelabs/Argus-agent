...
// kv "compression" holds this as JSON; missing keys fall back to defaults.
...
impl CompressionCfg {
  ...
  // Per-model overrides win by longest substring match: "llama3.1:8b" hits
  // before "llama3.1", both before the global threshold.
...
// Context window comes from the model's capabilities JSON (models.dev sync).
...
// Safety-net estimate: ~4 chars per token. Only used when the provider's real
// usage is unavailable, or as the second (est_ratio) check.
