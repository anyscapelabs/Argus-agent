          ...
          <thead>
            <tr className="bg-bg-secondary">
              {headRow.cells.map((cell, idx) => (
                <th key={idx} className="px-3 py-2 text-left font-medium text-text-primary">{cell.txt}</th>
              ))}
            </tr>
          </thead>
        )}
          ...
          {bodyRows.map((row, rIdx) => (
            <tr key={rIdx} className="border-t border-border-primary">
              {row.cells.map((cell, cIdx) => (
                <td key={cIdx} className="px-3 py-2 text-left font-light text-text-secondary">{cell.txt}</td>
              ))}
            </tr>
          ))}
        </tbody>
