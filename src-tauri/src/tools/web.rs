...
// One shared client: connection pool + UA survive across tool calls.
...
pub async fn search(args: &Value) -> Result<String, String> {
  ...
  if results.is_empty() {
    // DDG serves an anomaly/captcha page instead of results when it
    // suspects automation; surface that instead of an empty success.
    return Err("search returned no results (the engine may be rate-limiting automated queries — try again in a moment)".into());
  }
...
fn parse_results(html: &str) -> Vec<(String, String, String)> {
  ...
  // Snippets and anchors interleave 1:1 in the DDG markup; align by order.
...
pub async fn read(args: &Value) -> Result<String, String> {
  ...
  // Jina unavailable or empty: fetch the page ourselves and strip markup.
  let resp = client()
    .get(url)
    .send()
    .await
...
mod tests {
  ...
  fn html_to_text_drops_scripts() {
    let t = html_to_text("<html><script>var x=1;</script><style>a{}</style><body><p>hello world</p></body></html>");
