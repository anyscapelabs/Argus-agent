...
- Live turns render one panel too: `ChatDetailPage` builds steps from prior rows (done) + the live `turn.text` row (running, with `turn.term/termCode` outputs) and hides tools in the live `AgentBubble` — otherwise the in-flight action shows twice as "Ran" and "Running".
