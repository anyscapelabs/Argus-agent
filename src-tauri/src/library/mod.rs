...
// Library: generated files on disk under app_data/library/<yyyy-mm>/,
// DB index for list/search. Agent (MCP) and UI share these commands.
...
// Absolute path on disk, for the Open button (OS default app via opener plugin).
#[tauri::command]
pub fn library_path(gw: State<'_, Gateway>, id: String) -> Result<String, String> {
  let conn = gw.conn.lock().map_err(|e| e.to_string())?;
  let item = store::get(&conn, &gw.library_dir, &id)?;
  Ok(gw.library_dir.join(&item.path).to_string_lossy().into_owned())
}
