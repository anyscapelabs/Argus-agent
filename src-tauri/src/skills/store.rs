...
pub fn migrate(conn: &Connection) -> Result<(), String> {
  // legacy dev table had an id column; drop it and the fts index, rebuild fresh
  let legacy: i64 = conn
    .query_row(
...
fn chk_sizes(description: &str, body: &str) -> Result<(), String> {
  if description.len() > DESC_MAX || description.contains('\n') {
    return Err(format!("description must be one line, at most {DESC_MAX} bytes"));
  }
...
fn ftime(path: &Path) -> Result<i64, String> {
  ...
  Ok(meta.modified().map_err(|e| e.to_string())?.duration_since(UNIX_EPOCH).map(|d| d.as_secs() as i64).unwrap_or(0))
}

// Frontmatter: --- / name: / description: / --- / body. Minimal on purpose.
fn parse_md(content: &str) -> Result<(String, String, String), String> {
  ...
  if name.is_empty() || desc.is_empty() {
    return Err("frontmatter needs name + description".into()); // Drop it
  }
...
// Read the file, refresh its mirror row. Source/origin only set on first insert.
fn index_upsert(conn: &Connection, dir: &Path, name: &str) -> Result<(), String> {
  ...
  if fname != name {
    return Err(format!("frontmatter name '{fname}' must match filename '{name}'")); // Drop it
  }
  conn
    .execute(
...
// Rescan the folder, upsert changed files, drop rows whose file is gone.
pub fn sync(conn: &Connection, dir: &Path) -> Result<usize, String> {
  let mut seen: HashSet<String> = HashSet::new();
  let entries = match fs::read_dir(dir) {
    Ok(e) => e,
    Err(_) => return Ok(0), // no folder yet, nothing to do
  };
  for entry in entries {
    ...
    if chk_name(&name).is_err() || index_upsert(conn, dir, &name).is_err() {
      continue; // bad slug or bad file, drop it
    }
    seen.insert(name);
  }

  let all: Vec<String> = {
    let mut stmt = conn.prepare("SELECT name FROM skills").map_err(|e| e.to_string())?;
...
// Self-healing read: if the file changed on disk, re-index before answering.
pub fn get_skill(conn: &Connection, dir: &Path, name: &str) -> Result<Skill, String> {
  let path = md_path(dir, name);
  if !path.exists() {
    delete_row(conn, name)?;
    return Err(format!("skill '{name}' not found")); // Drop it
  }
  let on_disk = ftime(&path)?;
  let stored: Option<i64> = conn
    .query_row("SELECT file_mtime FROM skills WHERE name = ?1", params![name], |r| r.get(0))
    .optional()
    .map_err(|e| e.to_string())?;
...
// Agent refinement or UI edit: rewrite the file, refresh the mirror.
...
// key = name. The file is the truth, so deleting removes it.
