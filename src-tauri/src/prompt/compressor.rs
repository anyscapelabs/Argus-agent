...
// Best known usage: the provider's real tok_in when a call has happened,
// char-based estimate otherwise. needs_compact fires on either threshold.
...
fn utility_model(conn: &Connection) -> Result<String, String> {
  conn
    ...
    .map_err(|_| "no enabled model available for compaction".to_string()) // Drop it
}
...
pub async fn compact(gw: &Gateway, session_id: &str) -> Result<Option<CompactionRecord>, String> {
  ...
  if window.len() <= protect_first + protect_last {
    return Ok(None); // Drop it, nothing to compress
  }

  // Tail budget: threshold_tokens x target_ratio, walked from the newest
  // message backwards, never fewer than protect_last messages.
  let tail_budget = (context as f64 * cfg.threshold_for(model_id.as_deref().unwrap_or("")) * cfg.target_ratio) as i64;
  let mut acc = 0i64;
  ...
  if middle.len() == 0 || middle_tokens < 400 {
    return Ok(None); // Drop it, not worth a model call (and a cache bust)
  }
...
#[tauri::command]
pub async fn prompt_compact(gw: State<'_, Gateway>, session_id: String) -> Result<Option<CompactionRecord>, String> {
