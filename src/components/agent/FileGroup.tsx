...
export default function FileGroup({ blocks, diffs }: Props) {
  ...
  const files = blocks.map((b) => ({
    path: b.attrs.path ?? "file",
    action: b.attrs.action ?? "edited",
    type: b.attrs.type,
  }));
  ...
  return (
    <div className="mt-4 font-sans first:mt-0">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={
          "flex w-fit items-center gap-1.5 text-sm text-text-secondary " +
      ...
      {open && (
        <ul className="mt-1.5 flex flex-col gap-0.5">
          {files.map((f, idx) => {
            ...
            const hasDiff = add !== null;

            return (
              <li
                key={`${f.path}-${idx}`}
                className={
                  "flex cursor-pointer items-center gap-3 rounded px-1 py-1 " +
                  "text-sm transition-colors hover:bg-bg-secondary"
                }
              >
                ...
                {hasDiff && (
                  <span className="ml-auto flex shrink-0 items-center gap-1.5 font-mono text-xs">
                    ...
                    {rm !== null && rm > 0 && (
                      <span className="text-red-400">-{rm}</span>
                    )}
                  </span>
                )}
              </li>
