...
// Matches the icon set the Library page renders.
...
fn chk_name(name: &str) -> Result<(), String> {
  if name.is_empty() || name.len() > NAME_MAX {
    return Err(format!("name must be 1-{NAME_MAX} chars")); // Drop it
  }
  Ok(())
}

// Agent hands over a generated file; we copy it into the library and index it.
pub fn add(conn: &Connection, dir: &Path, item: &NewLibItem) -> Result<LibItem, String> {
  chk_name(&item.name)?;
  let src = Path::new(&item.source_path);
  if !src.is_file() {
    return Err(format!("source file '{}' not found", item.source_path)); // Drop it
  }
  let ext = src
    .extension()
    .and_then(|x| x.to_str())
  ...
  // Human-friendly name on disk, deduped with a counter, never overwrites.
  let file_name = sanitize(&item.name);
  let dot_ext = if ext == "bin" { String::new() } else { format!(".{ext}") };
  ...
  let sz = fs::metadata(abs_path(dir, &rel)).map_err(|e| e.to_string())?.len() as i64;
...
pub fn get(conn: &Connection, dir: &Path, id: &str) -> Result<LibItem, String> {
  let item = conn
    .query_row(&format!("SELECT {COLS} FROM library WHERE id = ?1"), params![id], row_item)
    .optional()
    .map_err(|e| e.to_string())?
  ...
  if !abs_path(dir, &item.path).exists() {
    ...
    return Err("item file missing, index row dropped".into()); // Drop it
  }
  Ok(item)
}
...
pub fn delete(conn: &Connection, dir: &Path, id: &str) -> Result<(), String> {
  ...
  let rel = match row {
    Some(r) => r,
    None => return Ok(()), // already gone, sorted
  };
...
// Startup pass: index files dropped in manually, drop rows whose file is gone.
pub fn sync(conn: &Connection, dir: &Path) -> Result<(), String> {
  ...
  for month in fs::read_dir(dir).map_err(|e| e.to_string())? {
    ...
    for entry in fs::read_dir(&month).map_err(|e| e.to_string())? {
      ...
      // Orphan file: index it under its filename
      let name = path.file_stem().and_then(|x| x.to_str()).unwrap_or("file").to_string();
      let ext = path.extension().and_then(|x| x.to_str()).unwrap_or("bin").to_lowercase();
  ...
  for rel in known {
    if !on_disk.contains(&rel) {
      conn.execute("DELETE FROM library WHERE path = ?1", params![rel]).map_err(|e| e.to_string())?;
    }
  }
  Ok(()) // Sorted
}
